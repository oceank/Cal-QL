...
# Calculate the distance (expected normalized error) between the critic's estimate and the MC estimate for each (s, a)
...
def main(argv):
    ...
    experiment_result_folder_name = f"ft_{algo}_{FLAGS.env}_seed{FLAGS.seed}_{expr_time_str}"
    ...
    alignment_episodes = 10000
    alignment_env = gym.make(FLAGS.env)
    alignment_sampler = TrajSampler(alignment_env.unwrapped, use_goal, use_mc=True, gamma=FLAGS.cql.discount, reward_scale=FLAGS.reward_scale, reward_bias=FLAGS.reward_bias,)
    alignment_replay_buffer = ReplayBuffer(alignment_episodes*alignment_env.spec.max_episode_steps)
    ...
    while True:
        ...
        if epoch == FLAGS.n_pretrain_epochs:
            # offline learning is done. need to differentiate with the case when loading a preprainted offline-optimized SAC
            alignment_trajs = alignment_sampler.sample(sampler_policy, n_trajs=alignment_episodes, deterministic=False, replay_buffer=alignment_replay_buffer)
            consumed_steps = sum([len(traj["actions"]) for traj in alignment_trajs])
            distance = calculate_distance(sac, alignment_trajs, next_rng())
            alignment_replay_buffer_fp = os.path.join(FLAGS.logging.output_dir, f"alignment_replay_buffer_{alignment_episodes}episodes_{consumed_steps}transitions_seed{FLAGS.seed}_{FLAGS.env}")
            alignment_replay_buffer.save_pickle(alignment_replay_buffer_fp)
            print(f"Collected {alignment_episodes} espisodes that consumes {consumed_steps} steps")
            print(f"Expected Normalized Error (Distance) Between Offline-Optimized Q-function and the True Q-function of the offline-optimized policy: {distance}")
            metrics['evaluation/Q_Pi_Distance'] = distance
            metrics['evaluation/alignment_steps'] = consumed_steps
            metrics['evaluation/alignment_episodes'] = alignment_episodes
            if FLAGS.save_model:
                save_data = {'sac': sac, 'variant': variant, 'epoch': epoch}
                save_data_fp = os.path.join(FLAGS.logging.output_dir, 'offline_optimized_model.pkl')
                wandb_logger.save_pickle(save_data, save_data_fp)



            # set up for the following online fine-tuning
            is_online = True
        ...
        with Timer() as eval_timer:
            if do_eval:
                ...
                if FLAGS.save_model:
                    save_data = {'sac': sac, 'variant': variant, 'epoch': epoch}
                    save_data_fp = os.path.join(FLAGS.logging.output_dir, 'model.pkl')
                    wandb_logger.save_pickle(save_data, save_data_fp) 
    ...
    if FLAGS.save_model:
        save_data = {'sac': sac, 'variant': variant, 'epoch': epoch}
        save_data_fp = os.path.join(FLAGS.logging.output_dir, 'finetuned_model.pkl')
        wandb_logger.save_pickle(save_data, save_data_fp)
